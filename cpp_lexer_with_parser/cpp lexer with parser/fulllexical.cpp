...
public:
    ...
    std::pair<TokenType, size_t> lookup(const string& identifier) {
        size_t index = 0;
        auto it = symbols.find(identifier);
        if (it != symbols.end()) {
            index = std::distance(symbols.begin(), it); // Calculate the index
            return { it->second, index };
        } else {
            return { TOKEN_UNKNOWN, 0 }; // Symbol not found
        }
    }

public:
    std::map<string, TokenType> symbols;
};
...
int main() {
    ...
    do {
        ...
        case TOKEN_IDENTIFIER:
            TokenType existingType;
            size_t index;
            std::tie(existingType, index) = symbolTable.lookup(token.lexeme);

            // If not found, insert it into the symbol table
            if (existingType == TOKEN_UNKNOWN) {
                symbolTable.insert(token.lexeme, token.type);
                existingType = token.type; // Update existing type
                index = symbolTable.symbols.size() - 1; // Index of the last element
            }

            cout << "Token: " << token.lexeme << ", Type: Identifier (Type: " << existingType << ", Index: " << index << ")" << std::endl;
            break;
